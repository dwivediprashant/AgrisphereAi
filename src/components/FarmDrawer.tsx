...
export function FarmDrawer({ onSave, onCancel }: FarmDrawerProps) {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    ...
                    toast.error("Could not access GPS. Defaulting to India center.");
    ...
    const handleSave = () => {
        if (drawnLayers.length === 0) {
            toast.error("Please draw a farm boundary first.");
            return;
        }
        ...
        if (layer instanceof L.Polygon) {
            ...
            toast.error("Please draw a Polygon or Rectangle.");
        }
    };

    if (!mapCenter) return <div className="p-8 text-center">Loading Maps...</div>;

    return (
        <Card className="p-4 space-y-4">
            <div className="flex justify-between items-center">
                <div>
                    <h3 className="font-bold text-lg">Draw Your Farm Boundary</h3>
                    <p className="text-sm text-muted-foreground">Use the polygon tool (⬠) to trace your field's exact edges.</p>
                </div>
            ...
            <div className="h-[500px] w-full border rounded-lg overflow-hidden relative">
                ...
                <div className="absolute bottom-4 left-4 z-[1000] bg-white/90 p-2 rounded shadow text-xs">
                    <strong>Tip:</strong> Click points to draw.<br />
