...
const PestPrediction = () => {
  ...
  const handleExplain = async () => {
    ...
    try {
      ...
      const dataSummary =
        t("pest.summary.crop", { crop }) +
        " " +
        t("pest.summary.weather", { temp, humidity, rainfall }) +
        " " +
        t("pest.summary.prediction", {
          name: result.primary_pest.pest_name,
          level: result.primary_pest.risk_level,
          score: result.primary_pest.risk_score,
        }) +
        " " +
        t("pest.summary.recommendation", {
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  <Label className="flex items-center gap-2">
                    <Thermometer className="h-4 w-4" /> {t("pest.temp")}
                  </Label>
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  <Label className="flex items-center gap-2">
                    <Droplets className="h-4 w-4" /> {t("pest.humidity")}
                  </Label>
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  <Label className="flex items-center gap-2">
                    <CloudRain className="h-4 w-4" /> {t("pest.rainfall")}
                  </Label>
                ...
                <Card className="border-l-4 border-l-blue-500 shadow-md">
                  ...
                  <CardContent>
                    <div className="text-center mb-6">
                      ...
                      <div
                        className={`text-5xl font-extrabold ${getRiskColor(result.primary_pest.risk_level)}`}
                      >
                        {result.primary_pest.risk_score}%
                      </div>
                      <div
                        className={`text-xl font-bold mt-2 ${getRiskColor(result.primary_pest.risk_level)}`}
                      >
                        {t("pest.riskLevel", {
                          level: result.primary_pest.risk_level,
                        })}
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h4 className="font-semibold text-gray-700 mb-2">
                        {t("pest.primaryThreat")}:{" "}
                        {result.primary_pest.pest_name}
                      </h4>
                      <p className="text-sm text-gray-600 mb-2">
                        <strong>{t("pest.recommendation")}:</strong>{" "}
                        {result.primary_pest.recommendation}
                      </p>
                    </div>
                  </CardContent>
                </Card>
